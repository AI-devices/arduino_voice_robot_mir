...
**Краткое описание работы:** находясь в активном режиме, модуль прослушивает голосовые команды пользователя. Если система распознала команду из списка, она произносит её через внешний динамик и отправляет сигнал на робот по нескольким интерфейсам.
...
ESP32-S3FH4R2, Espressif Systems

Xtensa® Dual-core 32-bit LX7 Microprocessor

Cores: 2; Clock frequency: up to 240 MHz;
...
Отправка данных в лог через USB-разъём (Type-C).
...
USB Type-C, 5 В, 500 мА (мин).

Либо с использованием контактов 3 - VCC, 5В; 4 - земля.

Средняя потребляемая мощность	30 mA.
...
В нашем распоряжении была четырехколёсная платформа с обычными счеточными двигателями, управляемыми через драйвер. Вы можете использовать AI-модуль для управления любыми устройствами на Ардуино - принцип работы и подключения не меняется, только скорректируйте свой скетч. 
